        ...
        if (
            ...
            if ($this->getInput()?->getOption('venv')) {
                ...
                if (!file_exists("$currentPath/.venv")) {
                    ...
                    $this->system("ln -s /usr/bin/python-config $pythonConfigPath", ignore: true);
                    // 软链python-include
                    $this->system("rm -rf $currentPath/.venv/include/python$this->pythonVersion", ignore: true);
                    $this->system("ln -s /usr/include/python$this->pythonVersion $currentPath/.venv/include", ignore: true);
                    $this->comment("Virtual environment created. Python-config path: $pythonConfigPath");
                } else {
